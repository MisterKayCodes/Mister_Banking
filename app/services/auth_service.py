...
from app.models.wallet import Wallet  # Mister, added the Crypto Vault model
from app.core.security import hash_password, verify_password, create_access_token
from app.core.ledger_tools import generate_account_number
from app.core.crypto import generate_realistic_address # Mister, added the Address Generator

# -------------------- REGISTRATION --------------------

def register_user(db: Session, full_name: str, email: str, password: str):
    """Mister, this creates the citizen and their dual-vault system."""
    ...
    # Mister, we wrap the creation in a try-block for 'Absolute Integrity'.
    try:
        # 1. Create the Citizen (The Master Record)
        user = User(
            full_name=full_name,
            email=email,
        ...
        db.add(user)
        # Mister, flush() is the secret sauce—it generates the user.id 
        ...
        # 3. The Crypto Vault (Using Mister's Crypto Engine)
        # ## Mister, 'the Citizen needs their keys generated on the fly.'
        ...
        # Mister, 'we seal the vault only when every piece is perfect.'
        db.commit()
        db.refresh(user)
        return user

    except Exception as e:
        db.rollback() # Mister, 'total wipeout if a single error occurs.'
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Mister, the system failed to build the vault: {str(e)}"
        )
...
def set_pin(db: Session, user_id: int, pin: str):
    """Mister, the second factor: The Transaction PIN."""
